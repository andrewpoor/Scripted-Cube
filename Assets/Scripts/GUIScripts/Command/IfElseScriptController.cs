...
public class IfElseScriptController : MonoBehaviour, IScriptController {
   ...
   public ElseScriptController elseBlock; //The else part of the block.
   ...
   public void Start() {
      ...
      GetComponent<CommandDetails> ().numBlocks = 5;
   }
   ...
   public void InsertIntoScript(List<GameObject> surroundingCommands) {
      ...
      elseBlock.SetColliderEnabled (true);
   }
   ...
   public void RemoveFromScript() {
      ...
      //When not in the script, disable collision detection for the rest.
      foot.GetComponentInChildren<BoxCollider2D> ().enabled = false;
      elseBlock.SetColliderEnabled (false);
   }
   ...
   public void UpdateVisuals() {
      ...
      //Add blocks for the else part.
      commandDetails.numBlocks += elseBlock.commandDetails.numBlocks;
   ...
   public string CollateScript() {
      //If the condition isn't present, default to false as a condition.
      ...
      scriptFooter = elseBlock.CollateScript ();
   ...
   public void SetScriptPosition(int position) {
      ...
      elseBlock.commandDetails.scriptPosition = position;
   ...
   private void UpdateScriptUI() {
      ...
      //Move else block to bottom of script.
      elseBlock.GetComponent<RectTransform>().anchoredPosition = new Vector2 (0.0f, -blockHeight * (Mathf.Max(blocks, 2) - 1));

      //Move foot to bottom of script.
      foot.GetComponent<RectTransform> ().anchoredPosition = new Vector2 (0.0f, -blockHeight * Mathf.Max(elseBlock.commandDetails.numBlocks, 1));
   }
